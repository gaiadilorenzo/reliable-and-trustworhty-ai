...
def fgsm_untargeted(model, x, label, eps, **kwargs):
    ...
    with torch.enable_grad():
        ...
        output = model(adv_x)
        loss = ce_loss(output, target=torch.tensor(label))
        loss.backward()
...
for epoch in range(1, args.num_epochs + 1):
    ...
    for batch_idx, (x_batch, y_batch) in enumerate(tqdm(train_loader)):
        ...
        if args.defense == 'PGD':
            ...
            model.train()
            x_adv = pgd(model, x_batch, y_batch, K, EPS, 2.5 * EPS / K, targeted=False, clip_min=0, clip_max=1.0)
            output = model(x_adv)
            ...
            x_adv = pgd(model, x_batch, y_batch, K, EPS, 2.5 * EPS / K, targeted=False, clip_min=0, clip_max=1.0)
            output = model(x_adv)
            loss_adv = ce_loss(output, y_batch)
            loss = loss_nat + LAMBDA * loss_adv
    ...
    for batch_idx, (x_batch, y_batch) in enumerate(tqdm(test_loader)):
        ...
        x_adv_prime = pgd(model, x_batch, y_batch, K, EPS, 2.5 * EPS / K, targeted=False, clip_min=0, clip_max=1.0)
        out_adv = model(x_adv_prime)
